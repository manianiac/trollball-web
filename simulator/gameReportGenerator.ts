...
import { match, match_progress, TEAM_NAMES } from "@/shared/types";
...
const generateContentHelper = async (
  ...
  if (useLocalAi) {
    console.log(
      `[AI] Attempting to generate content using local llama.cpp instance at ${localAiUrl}...`,
    );
    try {
      const body: any = {
        model: "local",
        messages: [
          { role: "system", content: NOK_THE_CORRUPTER_PERSONA },
      ...
      if (response.ok) {
        ...
        if (text) {
          console.log(
            "[AI] Successfully generated content using local llama.cpp.",
          );
          return text;
        }
      }
      ...
      console.warn(
        `[AI] Local AI request failed with status ${response.status}. Falling back to Gemini...`,
      );
    } catch (localError) {
      console.warn(
        "[AI] Failed to connect to local llama.cpp server. Falling back to Gemini...",
        localError,
      );
    }
  }
...
export async function generateGameReports(
  ...
  console.log("--- RAW AI RESPONSE STRING ---");
  console.log(result);
  console.log("------------------------------");
  ...
  try {
    ...
    const pre =
      parsed.preGameReport ||
      parsed.preGame ||
      parsed.pregame ||
      parsed.pre_game_report ||
      parsed.pre_game ||
      parsed.pregame_report;
    const post =
      parsed.postGameReport ||
      parsed.postGame ||
      parsed.postgame ||
      parsed.post_game_report ||
      parsed.post_game ||
      parsed.postgame_report;
    ...
    const stringifyIfNeeded = (val: any): string => {
      ...
      if (typeof val === "object") {
        return Object.values(val)
          .map((value) => stringifyIfNeeded(value))
          .join("\n\n");
      }
      return String(val);
    };
    ...
    return {
      preGameReport: stripEmojis(stringifyIfNeeded(pre)),
      postGameReport: stripEmojis(stringifyIfNeeded(post)),
      finalScore: parsed.finalScore || {
        homeTeam:
          typeof gameData.homeTeam === "object" && "name" in gameData.homeTeam
            ? gameData.homeTeam.name
            : "",
        homeScore: gameData.homeScore,
        awayTeam:
          typeof gameData.awayTeam === "object" && "name" in gameData.awayTeam
    ...
    console.error(
      "Error parsing AI response JSON, falling back to raw text:",
      e,
    );
    const stripEmojis = (str: string): string => {
      if (!str) return "";
    ...
    return {
      preGameReport: stripEmojis(result),
      postGameReport: stripEmojis(result),
      finalScore: {
        homeTeam:
          typeof gameData.homeTeam === "object" && "name" in gameData.homeTeam
            ? gameData.homeTeam.name
