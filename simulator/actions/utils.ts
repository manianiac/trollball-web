...
import { applyActiveModifiers } from "./modifiers";
...
export const calculateSuccess = (
  ...
  // 1. Apply active stadium/weather/other modifiers
  const { modifiedSkill: postModSkill, narrations } = applyActiveModifiers(
    gameState,
    activePlayer,
    action,
    modifiedSkill,
  );
  modifiedSkill = postModSkill;

  // Push narrations to the plays log
  if (narrations.length > 0) {
    gameState.plays.push(...narrations);
  }

  // 2. Apply alcohol/Ballmer Peak/drunk modifiers
  if (activePlayer && (activePlayer.stats.current_alcohol || 0) > 0) {
    const tolerance = activePlayer.stats.alcohol_tolerance;
    const current = activePlayer.stats.current_alcohol || 0;
    ...
    if (current <= tolerance) {
      ...
      modifiedSkill = modifiedSkill * (1 - penaltyPercent);
    }
  }
...
export const handleScore = (
  ...
  if (gameState.openBar) {
    ...
    const drink = (p: player) => {
      p.stats.current_alcohol =
        (p.stats.current_alcohol || 0) + getRandomInt(15, 30);
    };
    ...
    const checkStatus = (players: player[]) => {
      ...
      const alcohol = randomPlayer.stats.current_alcohol || 0;
    ...
    if (gameState.homeTeam.activePlayers)
      checkStatus(gameState.homeTeam.activePlayers);
    if (gameState.awayTeam.activePlayers)
      checkStatus(gameState.awayTeam.activePlayers);
  }
...
export const getAlcoholNarration = (player: player): string => {
  if (!player.stats.current_alcohol || player.stats.current_alcohol === 0)
    return "";
