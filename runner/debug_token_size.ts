...
import { popularity } from "../simulator/popularity";
import { heroesOfTheRealm } from "../simulator/heroes";

// --- Mocking load functions from createGames.ts ---

function loadAllGames(): match[] {
  const GAMES_DIR = path.join(process.cwd(), "runner", "results");
...
const loadAllRecaps = () => {
  const GAMES_DIR = path.join(process.cwd(), "simulator", "storage");
  if (!fs.existsSync(GAMES_DIR)) return [];
  const filenames = fs.readdirSync(GAMES_DIR);
